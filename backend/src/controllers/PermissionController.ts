...
static async createPermission(req: Request, res: Response) {
  try {
    ...
    if (!tacheId || !userId || !droit) return res.status(400).json({ error: "Tous les champs sont requis" });

    const tache = await prisma.tache.findUnique({ where: { id: parseInt(tacheId) } });
    if (!tache) return res.status(404).json({ error: "Tâche introuvable" });

    if (tache.userId !== currentUserId) return res.status(403).json({ error: "Vous ne pouvez accorder des permissions que sur vos propres tâches" });
    ...
    if (existingPermission) return res.status(409).json({ error: "Cette permission existe déjà" });
  ...
  try {
    ...
    if (isNaN(permissionId)) return res.status(400).json({ error: "ID de permission invalide" });
    ...
    if (permission.tache.userId !== currentUserId) return res.status(403).json({ error: "Vous ne pouvez supprimer que les permissions de vos propres tâches" });
